...
import type { ChannelRuntimeError, ChannelStatus } from "../channels/channel-runtime.ts";
...
const statusCommand: CommandHandler = {
  ...
  execute: (interaction, { client, guild }) =>
    Effect.gen(function* () {
      ...
      yield* Effect.tryPromise(() => interaction.editReply({ embeds: [createStatusEmbed(status)] }));
    }),
};
